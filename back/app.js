...
app.get("/:id_places/waitinglist", (req, res) => {
  database.query(
    "SELECT COUNT(id_user) AS total_de_clientes, id_places FROM `lista_espera` GROUP BY 2 HAVING id_places = ? ",
    req.params.id_places,
    (error, results) => {
      if (error) {
        console.log(error);
        res.status(404).send(error);
...
//Ruta para eliminar usuarios de la lista_espera de parte del manager
app.delete("/:id_places/remove/:id_user", (req, res) => {
  database.query(
    "DELETE FROM lista_espera WHERE id_user = ? AND id_places = ?",
    [req.params.id_user, req.params.id_places],
    (error, results) => {
